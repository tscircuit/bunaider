...
export async function fixPr(prNumber, repoInfo) {
  try {
    ...
    if (repoInfo.useOctokit) {
      ...
      pr = JSON.parse(execSync(`gh pr view ${prNumber} --json number,title,body,headRefName`).toString());
    }
    ...
    const aiderComments = comments
      ...
      .map(comment => comment.body.substring(7)); // Remove "aider: " prefix
    ...
    aiderMessage += "Please make the necessary changes to address the PR and review comments.";
